...
description: 用于用户提交论文、学位论文或参考文献清单后，系统审查参考文献真实性、题录准确性、文内—文后对应关系以及正文主张是否得到被引文献支持，并生成专业、清晰、可直接指导修改的论文引用审计报告（Markdown/Word）。用户要求“论文审计”“参考文献检查”“引用核对”“引用是否支持观点”“检查错引、过度推断或二手转引”时使用。开放主题综述、普通论文精读、论文代写或仅做语言润色时不使用。
---
...
- 将发现转化为作者可以直接执行的修改建议；
- 生成具有科研审稿质感的 Markdown 报告，并按需交付 Word 文档。
...
先完成一份高质量 Markdown 审查书。生成报告时读取并参考 `assets/report-template.md`，根据论文类型和实际发现调整栏目、顺序与篇幅，不要机械填充空章节。Markdown 定稿后，用 `write_file` 交付（用户要求 Word 时先用 `load_skill("docx")` 再用 `write_docx` 生成）；交付前回读主要内容，确认没有明显缺失、截断或链接错误。待核验条目与核验结果台账可用 `load_skill("xlsx")` + `write_xlsx` 交付 `.xlsx`（每条含 题录/DOI/核验状态/证据链接/结论），报告正文引用同源数据。

如果写入失败，保留完整 Markdown 报告并向用户明确说明失败原因。不得虚构文件路径或交付回执。

## 工具与核验方式

### 题录核验

当参考文献较多、需要批量核验题录时，使用 `search_web` / `web_fetch` 查询权威来源（Crossref、PubMed、arXiv、PMC、ACL Anthology、OpenReview、DOI 解析页、出版商官方页面）。工具只负责获取题录候选，不负责判断引用真实性、语义支持程度或审计结论。

可先整理一份待核验条目（内部笔记即可，例如）：
...
- 对已有 DOI 的条目直接打开/查询 DOI 解析页；
- 对有结构化标题的普通文献优先按标题与作者搜索；只有纯题录文本时才使用 Crossref 综合题录搜索；
- 对无 DOI 的普通文献保留少量候选；
- 对无 DOI 的软件、数据集和网页提示改查官方来源；
- 以 DOI 或标准化题录为键复用本次任务中的查询结果；
- 中断或重新运行时只补查未完成条目；
- 区分未找到、请求失败和应优先核验官方来源。

执行者根据候选的标题、作者、年份、载体和 DOI 自行判断匹配关系。不得把搜索结果的首个候选、相似度或 `ok` 状态直接写成“文献真实”“题录正确”或“支持正文主张”。查询中间结果保存在当前任务目录，不写入 Skill 全局数据库。
## 专家审计经验
...
3. 优先解决用户点名的问题；
4. 形成完整 Markdown 报告；
5. 用 `write_file`/`write_docx` 生成并回读交付文档；待核验条目较多时先用 `load_skill("xlsx")` + `write_xlsx` 建立核验台账再出报告；
6. 向用户交付总体结论、最重要发现和文件路径；写入失败则交付 Markdown 并说明原因。
...
8. 不用 HTML 替代用户要求的 Markdown/Word 文档。
...
- 强调按需获取全文证据与来源复用；
- 生成高级 Markdown 审查书，并在需要时交付 Word 文档；
- 采用高自由度、低形式约束、结果导向的执行方式。

---

> 本项目适配（不替代原文）：本环境无 Python/终端，`scripts/lookup_metadata.py` 已移除，题录核验改用 `search_web`/`web_fetch` 查 Crossref/PubMed/arXiv/DOI 等权威来源；报告交付用 `write_file`（Markdown）或 `load_skill("docx")` + `write_docx`（Word）；待核验条目台账用 `load_skill("xlsx")` + `write_xlsx`。原文七阶段审计流程、引用角色判断与证据边界规则全部保留。
