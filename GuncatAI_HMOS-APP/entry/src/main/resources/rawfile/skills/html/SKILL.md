...
> 本环境**无终端/Python 工具**：不执行任何命令、不做本地/云电脑环境判定、不运行截图或内嵌脚本。产物一律用 `write_file` 直接写出。
...
- **单文件自包含**：一个 `.html` 文件到手就可用。
  ...
  - **静态资源引用**：
    - 图片、音视频等**由元素 `src` 或 CSS `url()` 加载**的素材，先下载到 `assets/`（用 `download_file`）；生图搜图返回的 URL 会过期，不能直接写进 html。
    - 页面一律用相对路径引用工作区素材，如 `<img src="assets/hero.png">`，不写绝对路径；本项目不做 Base64 内嵌。
...
- **交付与发布**：
  - 本环境**不支持在线发布/可访问链接**。交付时用 `write_file` 写出 `.html` 并明确告知文件路径；若页面依赖同目录 `assets/`，提醒用户把 HTML 和 `assets/` 一起保存。
  - 交付说明如实写清：没实现的功能、用的是示例数据、做不了的能力。同一个产物只交付一个 `.html` 文件，不要额外附一份 Base64 自包含版。
...
**交付前自检一次，改动后再自检一次**。本环境无法运行浏览器/截图脚本，改为逐项人工核对：

1. **单文件自包含**：`.html` 无外部 `.js` / `.css` / 额外 HTML 依赖，打开即可用。
2. **图片路径**：所有图片都静态写在 `src` 属性或 CSS `url()` 里，且对应文件真实存在于工作区 `assets/`；没有经 JS 动态拼接的图片路径（否则交付后会裂图）。
3. **响应式**：宽屏/窄屏（手机）都阅读正常；双栏/图表在窄屏下正确堆叠，图表容器在移动端占满视口宽度，不出现「空图 / 一根线 / 坐标轴叠一起」。
4. **交互与控制台**：JS 无语法错误、无空引用；外部 JS 库来自 `jsDelivr`/`cdnjs` 且版本明确。
5. **数据保真**：每个数字都能从源数据算出、可追溯，无编造。
6. **发布能力**：本环境无发布/链接能力，交付时明确说明"未发布"，只给文件路径。

> 若以上某项无法在本环境验证（例如无法真机打开浏览器），如实说明，不得假装已自检通过。
...
**字体走自托管镜像** `https://miaoda.doc.cn/fonts/css2?family=…`：查询语法与 Google Fonts 的 `css2` 端点完全一致，返回的 `@font-face` 也指向自托管 CDN，两跳都不经过 Google。**不直连** `fonts.googleapis.com` **/** `fonts.gstatic.com`（部分地区不可达）。多字族就重复写多个 `family=`，例如 `<link rel="stylesheet" href="https://miaoda.doc.cn/fonts/css2?family=Noto+Serif+SC:wght@400;600;700;900&family=Noto+Sans+SC:wght@300;400;500;700&display=swap">`。每个 `font-family` 都要带完整的系统字体 fallback 栈，字体加载失败时页面仍然成立。
...
> 本项目适配：本环境无 Python/终端/浏览器工具，不执行 embed/shot 脚本、不做在线发布；交付用 `write_file` 写出 `.html` 并告知文件路径，素材用 `download_file` 下载到 `assets/`，配图用 `write_svg`。原文的 HTML 编写规范、视觉与防 slop 红线保留。