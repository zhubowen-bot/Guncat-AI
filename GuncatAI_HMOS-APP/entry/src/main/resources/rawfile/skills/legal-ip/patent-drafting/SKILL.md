...
本项目没有终端、子 agent，也没有平台级编排组件。材料审计、权利要求书、说明书、交付编译，四个阶段由同一个执行者在同一个对话里串行做完，全程共享同一份工作文件 `draft.md`——阶段一到阶段三往里面逐步填内容，交付编译阶段直接读这份文件并用 `write_docx` 生成 Word。分流由你判断，审计由你完成，权利要求由你撰写，自检由你执行，稿子由你改。不存在"这一步交给别的组件处理"这个说法：**读到这份 SKILL.md 的你，就是全流程唯一的执行者。**
...
| P0 | “本次定稿前需要确认” |
| P1 | “正式提交前建议确认” |
| 自检清单、检查编号、修稿循环 | 不展示；只用自然语言说明已完成格式与一致性检查 |
| 主流程/其他业务等流程标签 | “本次已完成”“本次未包含” |

内部编码只用于推理、独立的内部工作记录和自检清单；审阅说明、申请文件正文、最终回复属于**对外交付**，不得出现 A/B/C/D、P0/P1、自检编号、skill、脚本名或“主链/相邻业务”等内部词。不要把内部审计表整张贴给用户，应将结论改写为申请人可以直接判断和行动的语言。
...
| 交付编译 | 三阶段全部完成、`draft.md` 齐全之后 | 本文件下方"交付合同"一节（root 内联，不外包给子 skill） | 用 `write_docx` 生成的 docx |
...
实测教训：长流程末尾习惯性跳过"自检+生成 Word"这一步，写完文字就当交付完成。为了不让这件事在这个 skill 上重演：

**三个撰写阶段都做完、`draft.md` 已经汇总齐全之后，进入交付阶段的第一个动作是执行完整自检并生成 Word 文件，不是继续写文字，也不是先把内容念给用户看。**

本环境无终端/脚本，原 `scripts/patent_build.py` 已移除。交付编译改为：先加载 `docx` 技能，按 `sub-skills/specification/SKILL.md` 末尾的固定骨架把 `draft.md` 整理为最终申请文件文本，再用 `write_docx` 生成 docx；附图按 `sub-skills/specification/SKILL.md` 第 4 节约定嵌入（工作区图片路径 + Word 段落/表格）。

### 自检清单（替代原 build 检查，逐项人工核对）

生成 Word 前，按以下清单逐项核对 `draft.md`，发现问题先修稿再生成；**不允许跳过自检直接交付，也不允许绕开检查手工拼一个 docx**：

1. **C1 发明名称**：不超过 25 字，且与说明书/摘要中的名称一致。
2. **C2 权利要求编号**：从 1 开始连续；每条权利要求恰好一个句号且在末尾；无断句、无句号缺失。
3. **C3 从权引用关系**：从权引用编号小于自身；被引用的权项确实存在；多引用从权不引用多引用从权；从权引用他项时，句中限定对象的名称与被引权项里的主题名称一致（例如都叫“所述垫片本体”，不能一条叫“垫片”另一条叫“本体”）。
4. **C5 摘要**：不超过 300 字；发明名称不超过 25 字。
5. **C7 套话黑名单**：命中 `references/writing-style.md` 所列套话类别的一律改写，不能原样带入正文。
6. **C13 对外称谓纪律**：交付文档中无流程自指、工具自指、skill/技能包/脚本名等内部称谓。
7. **C14 摘要与发明内容强词一致**：摘要单方面出现“显著/彻底/经测试”等强词而发明内容未支撑的，降级或删除；正文与摘要同用强词时按措辞分寸表自查。
8. **格式一致性**：图号、标记、术语、标题层级全文一致；附图说明与说明书附图一一对应。
9. **事实分级**：材料外未新增技术事实；未测试的不写成已验证；范围必须是材料明确记载。

修稿循环可以反复多次，直到自检全部通过；自检结论记在内部工作记录中。

### 自检通过之后、生成 Word 之前：证据式找茬（只查三类语义残留）

以审查员视角把 `draft.md` 快速扫一遍，对三类自检正则抓不住的语义残留各给一行结论——**套话变体、流程自指、措辞越级**——每行要么“位置+原句摘录+改法”（回去改完再自检），要么写“无”。只有结论没有原句摘录的视为未检查。三项都干净就写三个“无”，**不许为走流程硬编问题**。结论记在你的工作过程里即可，不进交付正文。

找茬完成后，在内部工作记录中保存自检结论和三项语义复核结果，供排查问题时使用；不要把日志复制到对外回复。

发出最终回复前做一次核对：回复里写的“通过 N 项校验”与内部工作记录里实际自检项数一致，且自检确实在本轮完成过。数字对不上或本轮没做过自检，说明交付没有过门禁——回去补做，改完再发。

### 自检通过之后：最终回复内容

最终回复保持简短、自然，不逐字复述文档，并覆盖以下信息：

1. 说明专利申请文件审阅稿已经完成，给出 docx 文件名和路径；用一句自然的话说明校验情况，并**写明本次通过的自检项数**（例如“文件已通过全部 9 项格式与一致性人工自检”）。这个数字必须来自你本轮真实完成的自检，不得凭印象填写；自检原文、检查编号等内部日志不出现在回复里，但要保留在内部工作记录中。
...
**对外称谓纪律**（各阶段子文件开头逐字同文）：你是资深专利代理师，正在产出以代理机构口吻署名的法律文书。交付文本（文档与最终回复）中禁止任何流程自指与工具自指——对外一律称"本次撰写服务 / 本文件"，不得出现 skill、技能包、脚本名等内部称谓（如"本skill职责范围不包含检索"要写成"现有技术检索不在本次撰写服务范围内"）。文档侧 C13 自检项会拦，回复侧靠自控。

### 降级路径（仅当 Word 生成确实失败时使用）

降级有一个硬性前提：内部工作记录里已经保留了 `write_docx` 的完整失败信息。“环境不支持代码执行”这类一句话结论不构成降级理由——没有真实失败信息，就说明你还没有实际尝试过生成 Word，必须先尝试。拿到失败信息后，先检查 `docx` 技能是否已加载、文档结构是否合法、图片路径是否存在；确认本轮确实无法生成 Word 后，再使用以下降级交付：

1. 交付 `draft.md` 全文（直接呈现文本内容，不是 docx）；
2. 附一份手工自检清单，对着 C2、C3、C5 三项逐条人工核对，写出核对结果；
3. 在最终回复里用自然语言说明 Word 文件未能生成及原因摘要，不粘贴 traceback、命令输出或内部路径。

**任何情况下不得两手空空**：Word 生成成功就交付 docx；不能生成时交付文本审阅稿和自检清单，并说明 Word 生成失败。原始错误只保留在内部工作记录中。
...
> 本项目适配（不替代原文）：本环境无终端/脚本，原 `scripts/patent_build.py` 已移除，交付编译改为「人工自检清单 + `load_skill("docx")` + `write_docx` 生成 Word」；原 build 的 C2/C3/C5/C7/C13/C14 等检查改为人工逐项核对。若用户明确要“权利要求对照表/核验表”，可用 `load_skill("xlsx")` + `write_xlsx` 输出权利要求-说明书位置/特征对照表，但不得暴露 A/B/C/D、P0/P1 等内部编码。原文的三阶段主流程、子 skill 判据、红线清单、交付纪律与措辞规范全部保留。
