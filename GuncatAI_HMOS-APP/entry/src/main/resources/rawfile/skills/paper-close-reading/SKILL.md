...
description: 用于用户提供一篇或少量学术论文后，进行专业深度精读，讲清研究问题、研究故事、方法或理论机制、关键证据、实验结果、可信边界、复现风险与研究启示，并生成高级 Markdown 报告（可按需 Word）。用户要求“论文精读”“深度解读”“分析方法与实验”“判断论文价值或局限”时使用。开放主题综述、单纯题录核验、论文代写或语言润色时不使用。
---
...
- 生成具有专业研究咨询质感的 Markdown 精读报告，并按需交付 Word 文档。
...
- 没有种子论文的开放主题检索或系统综述（应使用 `research` 或 `deep-research` 类能力）；
- 只核验 DOI、题录、参考文献真实性或文内—文后对应（应使用 `reference-audit`）；
- 直接代写论文正文或只做语言润色或格式排版（应使用 `paper` 或 `content-rewrite`）；
- 只要求一句话摘要、翻译或普通问答且不需要深度分析（应使用通用能力）；
- 正式同行评审、编辑决策或研究不端调查（应使用 `llm-eval` 或 `paper-reviewer` 配合人工判断）
...
Markdown 定稿后，用 `write_file` 交付（用户要求 Word 时先用 `load_skill("docx")` 再用 `write_docx` 生成）；交付前回读标题、开头结论、一个关键数字或关键判断，确认没有明显缺失、截断、乱码或链接错误。关键图表/数字核对表可另用 `load_skill("xlsx")` + `write_xlsx` 输出（论文位置/图表号/关键数字/判断/来源页码），便于组会讨论与复现核对。

如果写入失败，保留完整 Markdown 报告并向用户说明真实原因。不得虚构文件路径或交付回执，也不要用 HTML 替代 Markdown/Word 文档。
...
- 用户要求 Word 时，一份内容一致、已回读检查的 Word 文档。

如果文档写入不可用，Markdown 报告仍是完整交付物，并附真实失败说明。
...
- 用 `write_file`/`write_docx` 生成并回读交付文档；
...
3. 优先解决用户点名的问题；
4. 形成完整 Markdown 报告；
5. 用 `write_file`/`write_docx` 生成并回读交付文档；
6. 向用户交付一句话判断、最重要洞察、报告文件和文件路径；写入失败时说明原因。
...
10. 不用 HTML 替代用户要求的 Markdown/Word 文档。
11. 不绕过付费墙、登录限制或版权保护。
...
- 生成高级 Markdown 精读报告，并在需要时交付 Word 文档；
- 采用高自由度、低形式约束、结果导向的执行方式。

---

> 本项目适配（不替代原文）：论文 PDF 用 `parse_document`/`pdf_to_images` 读取，Word/Markdown 用 `read_file`/`parse_document`；报告交付用 `write_file`（Markdown）或 `load_skill("docx")` + `write_docx`（Word），关键图表/数字核对表可用 `load_skill("xlsx")` + `write_xlsx`。原文七阶段精读流程、四类信息边界与研究行动导向全部保留。
