...
This directory contains **platform-adapted prompts and deployment resources**, ready for direct use in creating and running agents on platforms such as Tencent Yuanqi and ZhipuAI ChatGLM. The **Guncat 3.0 series (Pro / Flash / Mini)** and the **Guncat 3.1 series (Flash)** are platform-independent universal prompts and serve as the authoritative copy of the prompt library used by the Web and HarmonyOS `agents.json` clients.
...
├── Guncat 3.1/                          # 3.1-series universal prompts (Flash, CN + EN)
│   ├── Guncat 3.1-Flash_prompt_ZH_CN.md
│   └── Guncat 3.1-Flash_prompt_EN.md
...
- Except for `Guncat 3.0/` and `Guncat 3.1/`, prompts in this directory **contain platform-specific syntax, tool names, and invocation conventions**; direct migration to other platforms may result in inconsistent behavior.
- The prompts in `Guncat 3.0/` and `Guncat 3.1/` are **universal (platform-independent)** and kept in sync with the identically named prompts in [Guncat AI Web for API](../Guncat%20AI%20Web%20for%20API/) and both HarmonyOS apps' `rawfile/`; sync them when modified to avoid any client loading outdated versions.
