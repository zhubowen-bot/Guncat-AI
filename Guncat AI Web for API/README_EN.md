...
2026.8.31 Added **Mermaid diagram rendering** (flowchart / sequence / mindmap and all other types): ```mermaid code blocks in answers are automatically rendered as SVG diagrams after the stream finishes (lazy-loads mermaid@11 from CDN, theme follows the system dark/light mode); on render failure or CDN unavailability the source code is kept as-is, and the Markdown source remains copyable via the message actions
